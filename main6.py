...
def loop_game_screen():
    ...
    if init_game:
        ...
        max_time = font.render("Max Time: "+str(game.get_n_discs()*3)+"s", True, color.BLACK)
        if not final_time==0:
            player_time = font.render("Player Time: "+str(final_time//60)+"s", True, color.BLACK)
        else:
            player_time = font.render("Player Time: "+str(time//60)+"s", True, color.BLACK)
        screen.blit(player_moves, [20, 80])
        #screen.blit(min_moves, [20, 110])
        screen.blit(max_time, [20, 110])
        screen.blit(player_time, [20, 140])
        if game.get_n_discs()*3*60 < time:#moves_counter > game.min_moves:
            game_over = True
            if final_time==0:
                final_time = time
        ...
        if game_over:
            ...
            if len(game.positions[2].discs) == game.n_discs:
                if moves_counter == game.min_moves or True:
        ...
        if event.type == pygame.QUIT:
            ...
            if init_game:
                ...
                for i in range(0,1000):
                    if len(menu.btn_discs)>i and menu.btn_discs[i].is_clicked():
